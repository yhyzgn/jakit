...
public class AESFactory implements CryptoFactory {
    ...
    public SymmetricAlgorithm symmetric(String key, String iv, Mode mode, Padding padding, Format format) {
        return new AbsSymmetricAlgorithm<AES>(key, iv, mode, padding, format) {
            @Override
            public AES algorithm(Mode mode, Padding padding, byte[] key, byte[] iv) {
                return new AES(mode.name(), padding.getValue(), key, iv);
            }
        };
    }
}
